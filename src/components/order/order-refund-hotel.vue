<template>
  <div class="order_refund_hotel">
    ...
    <div class="order_refund_hotel_wrap" v-if="order.status != 2">
...
export default {
  ...
  methods: {
    ...
    save(){
      ...
      this.$axios({
        method: 'get',
        url: '/api/updateHotelOrderStatus.service?token=' + token + '&pre_id=' + _this.preId + '&status=6'
      }).then((res) => {
        if (res.status === 200) {
          _this._getDetail()
        }
